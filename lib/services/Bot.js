...
var Bot = function (_EventEmitter) {
  ...
  function Bot(options) {
    ...
    _this.modPluginsPath = options.modPluginsPath || _path2.default.join(process.cwd(), 'lib/mod-plugins');
  ...
  _createClass(Bot, [{
    ...
    value: function login() {
      ...
      client.on('message', function (msg) {
        ...
        } else if (msg.content.startsWith(_this3.config.admin_prefix)) {}
      });
    ...
    value: function runPlugins() {
      for (var plugin in this.plugins) {
        for (var command in this.plugins[plugin]) {
    ...
    value: function reloadPlugins() {
      ...
      Object.keys(require.cache).forEach(function (key) {
        if (key.startsWith(_this4.pluginsPath) || key.startsWith(_this4.modPluginsPath)) (0, _clearRequire2.default)(key);
      });
